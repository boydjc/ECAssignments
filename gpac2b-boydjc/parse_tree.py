...
class Node():
	...
	def execute(self, state, cache=None):
		...
		if(self.value == 'G'):
			...
			if('minPlayerDist' in cache):
				...
				playerDist, cache = self.playerDist(state, cache)
	...
	def playerDist(self, state, cache):

		pacPlayerLocations = [state['players'][key] for key in state['players'].keys() if 'm' in key]

		playerDistances = []

		for locCount in range(1, len(pacPlayerLocations)):
			playerDistances.append(sum(abs(val1-val2) for val1, val2 in zip(pacPlayerLocations[0], pacPlayerLocations[locCount])))
