...
def play_GPac(pac_controller, ghost_controller, game_map=None, **kwargs):
    '''
        ...
        for player in game.players:
            ...
            if 'm' not in player:
                if ghost_controller is None:
                    ...
                    for state in s_primes:
                        cache = {}
                        score, _ = ghost_controller.gene.execute(player, state, cache)

                        stateScores.append(score)

                    selected_action_idx = stateScores.index(max(stateScores))
            ...
            # select Pac-Man action(s) using provided strategy 
                ...
                if pac_controller is None:
                    ...
                    for state in s_primes:
                        cache = {}
                        score, _ = pac_controller.gene.execute(player, state, cache)
